...
def _patch_qwen3vl_capture_safety() -> None:
  ...
  if qwen_cls is not None and not getattr(qwen_cls, "_openpilot_generate_kwargs_patch", False):
    original_generate = qwen_cls.generate
    # Alpamayo1_5.forward()/sample_trajectories_from_data_with_vlm_rollout() spreads its own
    # tokenized_data dict straight into self.vlm.generate(**tokenized_data). tokenized_data
    # carries adapter-only bookkeeping (trajectory-fusion/cache-position/precomputed-feature
    # signaling) that the base Qwen3VL VLM never declared in forward()/prepare_inputs_for_generation(),
    # so HF's _validate_model_kwargs rejects it before generation starts. Strip it here.
    _openpilot_non_vlm_generate_kwargs = (
      "_skip_traj_fusion",
      "_openpilot_fused_input_ids_signature",
      "_openpilot_precomputed_image_features",
      "cache_position_seed",
    ...
    def generate(self, *args, **kwargs):
      for key in _openpilot_non_vlm_generate_kwargs:
        kwargs.pop(key, None)
      return original_generate(self, *args, **kwargs)

    qwen_cls.generate = generate
    qwen_cls._openpilot_generate_kwargs_patch = True
