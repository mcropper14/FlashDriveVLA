...
from openpilot.selfdrive.modeld.semantic_fusion import apply_semantic_fusion_generic, apply_semantic_lateral_overrides
...
def main(demo=False):
  ...
  while True:
    ...
    if run_count % model.constants.MODEL_FREQ == 0:
      alpamayo_enabled = params.get_bool("AlpamayoEnabled")
    ...
    if model_output is not None:
      ...
      if alpamayo_enabled and sm.seen['semanticPlan']:
        try:
          model_output, fusion_result = apply_semantic_fusion_generic(model_output, sm['semanticPlan'], model.constants, Plan)
          if fusion_result.applied:
            fused_curvature = get_curvature_from_plan(
              model_output['plan'][0, :, Plan.T_FROM_CURRENT_EULER][:, 2],
              model_output['plan'][0, :, Plan.ORIENTATION_RATE][:, 2],
              model.constants.T_IDXS,
              v_ego,
              lat_delay + DT_MDL,
            )
            model_output = apply_semantic_lateral_overrides(model_output, Plan, fused_curvature)
            if run_count % (5 * model.constants.MODEL_FREQ) == 0:
