...
def main(demo=False):
  ...
  while True:
    ...
    if run_count % ModelConstants.MODEL_FREQ == 0:
      alpamayo_enabled = params.get_bool("AlpamayoEnabled")
    ...
    if model_output is not None:
      ...
      if alpamayo_enabled and sm.seen['semanticPlan']:
        try:
          model_output, fusion_result = apply_semantic_fusion_generic(model_output, sm['semanticPlan'], ModelConstants, Plan)
          if fusion_result.applied:
            ...
            if run_count % (5 * ModelConstants.MODEL_FREQ) == 0:
